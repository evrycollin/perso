...
public class PathExecutor {
	...
	String idStr = Cypher.unmaskId(rootEntity.getType().getSimpleName(), tokens.remove(0));
	...
	if (nav instanceof CollectionAttribute) {
	    String idStr = Cypher.unmaskId(  targetEntity.getType().getSimpleName(), tokens.remove(0)) ;
	...
	if (entityInstance.getLink()!=null ) {
	    Field field = entityInstance.getEntity().getFields().get(entityInstance.getLink());
